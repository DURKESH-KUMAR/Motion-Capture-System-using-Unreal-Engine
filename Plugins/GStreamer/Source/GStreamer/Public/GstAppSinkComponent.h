...
public:	
	...
	UPROPERTY(Category="GstAppSink", EditAnywhere, BlueprintReadWrite)
	bool AppSinkEnabled = true;

	UFUNCTION(Category="GstAppSink", BlueprintCallable)
	UTexture2D* GetTexture() { return Texture->GetTextureObject().Get(); }
...
protected:
	...
	TSharedPtr<IGstAppSink> AppSink = nullptr;
	TSharedPtr<FGstTexture> Texture = nullptr;
