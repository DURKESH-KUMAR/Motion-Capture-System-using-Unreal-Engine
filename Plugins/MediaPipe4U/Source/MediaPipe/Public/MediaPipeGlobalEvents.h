...
#define DECLARE_GLOBAL_EVENT(OwningType, EventName) \
public: \
	DECLARE_EVENT(EventName, F##EventName##Event) \
	inline void Publish##EventName() \
	{ \
	...
	\
	template <typename UserClass, typename... VarTypes> \
	inline FDelegateHandle Subscribe##EventName##SP(UserClass* InUserObject, typename TMemFunPtrType<false, UserClass, void (VarTypes...)>::Type InFunc, VarTypes... Vars) \
	{ \
		Unsubscribe##EventName(InUserObject); \
		return EventName##Event.AddSP(InUserObject, InFunc, Vars...); \
	} \
...
{
	DECLARE_GLOBAL_EVENT(FMediaPipeGlobalEvents, M4ULicenseRefreshed)
