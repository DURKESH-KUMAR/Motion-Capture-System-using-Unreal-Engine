...
public:
	...
	bool RegisterFactory(const FName& ProviderName, const TSharedPtr<IWebcamProviderFactory>& Factory, bool bAsDefault = false);
	bool UnregisterFactory(const FName& ProviderName);
	FName GetDefaultProvider() const;
	FName GetProviderOrDefault(const FName& ProviderName) const;
private:
	FName DefaultProvider = NAME_None;
